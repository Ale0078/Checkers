...
namespace TileView
{
    public class TileGrid : Grid
    {
        ...
        {
            if (visualAdded is Tile || visualRemoved != null)
            {
        ...
        protected override void OnMouseLeftButtonUp(MouseButtonEventArgs e)
        {
            ...
            if (!(e.Source is TilePlaceholder))
            {
                return;
            }
        ...
        private Direction SetDirection(Direction searchingDirection, GridPosition droppedTilePosition, GridPosition sourcePosition)
        {
            ...
            else if (droppedTilePosition.DoesMatch(0, 0) || droppedTilePosition.DoesMatch(null, sourcePosition.Column))
            {
        ...
        private bool HaveTileAtColumn(GridPosition placeholderPosition)
        {
            for (int i = 0; i < RowDefinitions.Count; i++)
            {
                if (!(GetTileByPosition(this, new GridPosition(i, placeholderPosition.Column)) is TilePlaceholder))
                {
                    return true;
                }
            }
        ...
        private struct GridPosition
        {
            ...
            public GridPosition GetNextPosition(Direction direction = Direction.Right)
            {
                switch (direction)
                {
                    case Direction.Left:
